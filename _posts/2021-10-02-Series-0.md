...
* Growth probability - Each point that is selected has a probability for growing or decaying. This probability is biased in a particular, random direction. There is a 20% chance of the ball shrinking on average and five general categories: fast growth, slow growth, slow decay, fast decay.
...
Code has been stored on the blockchain [tx hash]() and uploaded to IPFS with the CID: [QmS6NHG7xH6CDSrwWzH35Uy2Yuc65FBacFXDaMbHzwvAdh](https://gateway.pinata.cloud/ipfs/QmS6NHG7xH6CDSrwWzH35Uy2Yuc65FBacFXDaMbHzwvAdh)

<code class="language-java">
String rs = "SEED";
String d = rs.replaceAll("[^0-9]", "").replaceAll("^0+(?!$)", "");
...
void setup(){
    ...
    smooth();
    frameRate(30);
    int sb = Math.round( random(2) )+4;
    rad = random(20)+ 40;
    largest_norm = rad;
    rix = random(0.001, 100000);
    ps = 10;
...
void draw() {
    ...
    endShape();
    float sd = 1 -(0.00001*sq(largest_norm - rad));
    float pw = sg * sd;
    if (frameCount < fr) {
        for (int i = 0; i < np; i++) {
            float fj = random(0.1);
            int r_ind = Math.round( noise(rix*fj*i*(fr-frameCount) )*vs.size() );
    ...
    else if (frameCount>=fr && frameCount < (fr+fp)) {
    }
    else if ( frameCount >= (fr+fp) && frameCount <= (fr+fp+fc) ) {
        if (frameCount == fr+fp){
